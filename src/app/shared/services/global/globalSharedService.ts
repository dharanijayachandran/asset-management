...
export class globalSharedService {
  ...
  rtDataSharingTopics: any[];
  assetTypeByCategorys: any;
  assetTypeByCategoryMap: Map<number, any[]>;
  setTabNameToAlarmConfig: string;
  setTabBoolean: boolean;
  clickToViewData: any;
  assetTagName: any;

  alarmConfigSetStateView: any;
  ...
  setTabName(data) {
    this.setTabNameToAlarmConfig = data;
    this.setTabBoolean = true;
  }

  setViewData(data) {
    this.clickToViewData = data;
  }
  // When click on update/view, ID will pass to respective update view
  GettingId(id) {
    this.selectedId = id;
    setTimeout(() => {
  ...
  serialNumberGenerateForAssetTags(array) {
    let i = 1;
    let returnModifiedArray = array.map((object) => {
      object.sNo = i;
      i++;
  ...
  setRtDataSharingTopics(rtDataSharingTopics) {
    this.rtDataSharingTopics = rtDataSharingTopics;
  }
  ...
  setAssetTypeByAssetCategoryId(assetTypeByCategoryMap: Map<number, any[]>) {
    this.assetTypeByCategoryMap = assetTypeByCategoryMap;
  }
